...
class Dashboard(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(header, text="MorseRelay", font=ctk.CTkFont(size=24, weight="bold"), text_color=t["text"]).pack(side="left")
        ...
        self.status_label = ctk.CTkLabel(header, text="Initializing…", font=ctk.CTkFont(size=12), text_color=t["secondary"])
        ...
        ctk.CTkLabel(id_card, text="Callsign", font=ctk.CTkFont(size=11), text_color=t["secondary"]).pack(anchor="w", padx=12, pady=(8, 0))
        ...
        ctk.CTkLabel(conn_card, text="Target IP", font=ctk.CTkFont(size=11), text_color=t["secondary"]).pack(anchor="w", padx=12, pady=(8, 0))
        ...
        self.test_btn = ctk.CTkButton(audio_card, text="♪ Test Tone", height=28, corner_radius=6, font=ctk.CTkFont(size=12), fg_color=t["card_hover"], text_color=t["text"], border_width=1, border_color=t["border"], command=self._test_tone)
        ...
        ctk.CTkCheckBox(speed_card, text="Auto-timing from WPM", font=ctk.CTkFont(size=11), text_color=t["secondary"], fg_color=t["accent"], variable=self.auto_timing_var, command=self._on_auto_timing_toggle).pack(anchor="w", padx=12, pady=(4, 0))
        ...
        ctk.CTkLabel(key_card, text="Assign any key to tap out messages. Each computer uses its own local key.", font=ctk.CTkFont(size=11), text_color=t["secondary"]).pack(anchor="w", padx=12, pady=(8, 4))
        ...
        self.tap_key_btn = ctk.CTkButton(tap_frame, text=KeyboardHookManager.get_key_display_name(self.settings.send_key), height=36, corner_radius=8, fg_color=t["bg"], text_color=t["text"], border_width=1, border_color=t["border"], font=ctk.CTkFont(size=14, weight="bold"), command=self._capture_key)
        ...
        ctk.CTkLabel(tap_frame, text="Tip: Turn on Tap Mode below, then hold this key. (Short press = Dot, Long press = Dash)", font=ctk.CTkFont(size=10), text_color=t["faded"]).pack(anchor="w", pady=(4, 10))
        ...
        self.morse_preview = ctk.CTkLabel(msg_card, text="", font=ctk.CTkFont(family="Consolas", size=14), text_color=t["dot"], anchor="w", justify="left", wraplength=640)
        self.morse_preview.pack(fill="x", padx=12, pady=(0, 4))
        self.tap_display = ctk.CTkLabel(msg_card, text="", font=ctk.CTkFont(family="Consolas", size=14), text_color=t["dot"], anchor="w", justify="left", wraplength=640)
        self.tap_display.pack(fill="x", padx=12, pady=(0, 4))

        # Auto-Send Row
        auto_frame = ctk.CTkFrame(msg_card, fg_color="transparent")
        auto_frame.pack(fill="x", padx=12, pady=(2, 4))
        ...
        self._slider_row(popup_card, "Auto-hide after", "duration_slider", 0, 30, self.settings.popup_duration, "s", self._on_duration_change, pad_bottom=10)
    ...
    def _card(self, parent, title: str) -> ctk.CTkFrame:
        t = self.theme
        card = ctk.CTkFrame(parent, corner_radius=10, fg_color=t["card"], border_width=1, border_color=t["border"])
        ctk.CTkLabel(card, text=title, font=ctk.CTkFont(size=11, weight="bold"), text_color=t["secondary"]).pack(anchor="w", padx=12, pady=(8, 0))
        return card
    ...
    def _on_msg_type(self, event=None):
        ...
        # Only start auto-send timer if we are in Text Mode (not Tap Mode)
    ...
    def _update_tap_display(self, morse: str):
        ...
        # Start auto-send timer if we are in Tap Mode and have morse
    ...
    def _tick_auto_send(self):
        ...
        # Check if there's still content to send
        has_content = False
        if self._tap_mode_active:
    ...
    def _on_auto_send_toggle(self):
        ...
        if self.settings.auto_send_enabled:
            self._on_net_status("Auto-Send ON")
        else:
            self._on_net_status("Auto-Send OFF")
            self._stop_auto_send_timer()
        self._on_msg_type()
    ...
    def _on_auto_delay_change(self, v):
        ...
        self.settings.save()
        # Reset timer with new delay if currently typing
        self._on_msg_type()

    def _send_message(self):
        # Cancel any pending auto-send timer since we are sending now
        self._stop_auto_send_timer()
        ...
        if self._tap_mode_active:
            ...
            if not morse: 
                self._on_net_status("Nothing to send.")
                return
        else:
        ...
        if not target:
            self._on_net_status("Send failed: No Target IP set!")
            return

        cs = self.callsign_entry.get().strip().upper()
        valid, msg = validate_callsign(cs)
        if not valid:
            self._on_net_status(f"Send failed: {msg}")
            return
    ...
    def _toggle_tap_mode(self):
        ...
        if self._tap_mode_active:
            ...
            # If switching to tap mode, stop text box auto-send
            self._stop_auto_send_timer()
        else:
            self.tap.stop()
    ...
    def _show_popup(self, callsign: str, morse: str, text: str, timestamp: float, conflict: bool = False):
        ...
        display_text = text
        if conflict: display_text = f"[CONFLICT] {text}"
...
class MiniWindow(ctk.CTkToplevel):
    ...
    def _build_ui(self):
        t = self.theme
        frame = ctk.CTkFrame(self, corner_radius=14, fg_color=t["card"], border_width=1, border_color=t["border"])
        frame.pack(fill="both", expand=True, padx=2, pady=2)

        header = ctk.CTkFrame(frame, fg_color="transparent")
        header.pack(fill="x", padx=12, pady=(8, 2))
        ctk.CTkLabel(header, text=self.settings.callsign, font=ctk.CTkFont(size=13, weight="bold"), text_color=t["text"]).pack(side="left")
        
        expand_btn = ctk.CTkLabel(header, text="⤢", font=ctk.CTkFont(size=16), cursor="hand2", text_color=t["secondary"])
        expand_btn.pack(side="right")
        expand_btn.bind("<Button-1>", lambda e: self._expand())

        ctk.CTkFrame(frame, fg_color=t["border"], height=1).pack(fill="x", padx=12, pady=2)

        self.morse_label = ctk.CTkLabel(frame, text="", font=ctk.CTkFont(family="Consolas", size=16, weight="bold"), text_color=t["dot"], anchor="center", wraplength=280)
        self.morse_label.pack(fill="x", padx=12, pady=(8, 2))
        
        self.text_label = ctk.CTkLabel(frame, text="", font=ctk.CTkFont(family="Consolas", size=11), text_color=t["faded"], anchor="center")
        self.text_label.pack(fill="x", padx=12, pady=(0, 4))

        self.tap_indicator = ctk.CTkLabel(frame, text=f"Hold {KeyboardHookManager.get_key_display_name(self.settings.send_key)} to tap", font=ctk.CTkFont(size=10), text_color=t["secondary"])
        self.tap_indicator.pack(pady=2)

        btn_frame = ctk.CTkFrame(frame, fg_color="transparent")
        btn_frame.pack(fill="x", padx=12, pady=(2, 10))
        ctk.CTkButton(btn_frame, text="Send ▸", height=28, corner_radius=6, font=ctk.CTkFont(size=12, weight="bold"), fg_color=t["accent"], text_color=t["bg"], command=self._send).pack(side="left", fill="x", expand=True, padx=(0, 4))
        ctk.CTkButton(btn_frame, text="Clear", height=28, width=60, corner_radius=6, font=ctk.CTkFont(size=11), fg_color=t["card_hover"], text_color=t["text"], border_width=1, border_color=t["border"], command=self._clear).pack(side="left")
