...
from morse import MorseTranslator, MorseMessage, TapModeController, validate_callsign
...
class Dashboard(ctk.CTk):
    """The main dashboard — full settings + message composition."""
    ...
    def _build_ui(self):
        ...
        id_card.pack(side="left", fill="x", expand=True, padx=(0, 6))

        ctk.CTkLabel(id_card, text="Callsign",
                     font=ctk.CTkFont(size=11),
                     text_color=t["secondary"]).pack(anchor="w", padx=12, pady=(8, 0))

        callsign_frame = ctk.CTkFrame(id_card, fg_color="transparent")
        callsign_frame.pack(fill="x", padx=12, pady=4)

        self.callsign_entry = ctk.CTkEntry(
            callsign_frame, height=34, corner_radius=8,
        ...
        self.callsign_status = ctk.CTkLabel(
            callsign_frame, text="✓", font=ctk.CTkFont(size=14),
            text_color=t["faded"],
        )
        self.callsign_status.pack(side="left")

        self.my_ip_label = ctk.CTkLabel(
            id_card, text=f"My IP: {self.network.get_local_ip()}",
            font=ctk.CTkFont(size=11),
            text_color=t["faded"],
        )
        self.my_ip_label.pack(anchor="w", padx=12, pady=(0, 10))

        # Connection card
        conn_card = self._card(row1, "Connection")
        conn_card.pack(side="left", fill="x", expand=True, padx=(6, 0))

        ctk.CTkLabel(conn_card, text="Target IP",
                     font=ctk.CTkFont(size=11),
                     text_color=t["secondary"]).pack(anchor="w", padx=12, pady=(8, 0))

        self.target_entry = ctk.CTkEntry(
            conn_card, height=34, corner_radius=8,
            fg_color=t["bg"], border_color=t["border"],
            text_color=t["text"], border_width=1,
            font=ctk.CTkFont(size=13),
            placeholder_text="192.168.1.x",
        )
        self.target_entry.pack(fill="x", padx=12, pady=4)
        if self.settings.target_ip:
            self.target_entry.insert(0, self.settings.target_ip)
        ...
        ctk.CTkLabel(port_frame, text="Port:",
                     font=ctk.CTkFont(size=12),
                     text_color=t["secondary"]).pack(side="left")

        self.port_entry = ctk.CTkEntry(
            port_frame, width=70, height=28, corner_radius=6,
            fg_color=t["bg"], border_color=t["border"],
            text_color=t["text"], border_width=1,
        ...
        self.listen_btn = ctk.CTkButton(
            port_frame, text="Listening ●", width=90, height=28,
        ...
        self.peers_label = ctk.CTkLabel(
            conn_card, text="Discovered: scanning…",
            font=ctk.CTkFont(size=10),
            text_color=t["faded"],
        )
        self.peers_label.pack(anchor="w", padx=12, pady=(0, 10))
        ...
        audio_card.pack(side="left", fill="x", expand=True, padx=(0, 6))

        self._slider_row(audio_card, "Frequency", "freq_slider",
                         200, 2000, self.settings.frequency,
                         " Hz", self._on_freq_change)
        self._slider_row(audio_card, "Volume", "vol_slider",
                         0, 100, int(self.settings.volume * 100),
                         "%", self._on_vol_change)

        self.test_btn = ctk.CTkButton(
            audio_card, text="♪ Test Tone", height=28, corner_radius=6,
            font=ctk.CTkFont(size=12),
            fg_color=t["card_hover"], text_color=t["text"],
            border_width=1, border_color=t["border"],
            command=self._test_tone,
        )
        self.test_btn.pack(fill="x", padx=12, pady=(4, 10))

        speed_card = self._card(row2, "Speed & Timing")
        speed_card.pack(side="left", fill="x", expand=True, padx=(6, 0))

        self._slider_row(speed_card, "WPM", "wpm_slider",
                         5, 40, self.settings.wpm,
                         "", self._on_wpm_change)

        self.wpm_info = ctk.CTkLabel(
            speed_card, text=f"1 dot = {1200/self.settings.wpm:.0f} ms",
            font=ctk.CTkFont(size=10),
            text_color=t["faded"],
        )
        self.wpm_info.pack(anchor="w", padx=12)

        self.auto_timing_var = ctk.BooleanVar(value=self.settings.auto_timing)
        auto_chk = ctk.CTkCheckBox(
            speed_card, text="Auto-timing from WPM",
            font=ctk.CTkFont(size=11),
            text_color=t["secondary"],
            fg_color=t["accent"], text_color_disabled=t["faded"],
            variable=self.auto_timing_var,
            command=self._on_auto_timing_toggle,
        )
        auto_chk.pack(anchor="w", padx=12, pady=(4, 0))

        self._slider_row(speed_card, "Dot threshold", "dot_slider",
                         50, 500, self.settings.dot_threshold,
                         " ms", self._on_dot_change)
        self._slider_row(speed_card, "Letter gap", "letter_slider",
                         100, 800, self.settings.letter_gap,
                         " ms", self._on_letter_gap_change)
        self._slider_row(speed_card, "Word gap", "word_slider",
                         300, 1500, self.settings.word_gap,
                         " ms", self._on_word_gap_change)

        # ── Row 3: Key Assignment ──
        key_card = self._card(self.scroll, "Tap Key Assignment")
        key_card.pack(fill="x", padx=20, pady=4)

        ctk.CTkLabel(
            key_card, text="Assign any key to tap out messages. Each computer uses its own local key.",
            font=ctk.CTkFont(size=11), text_color=t["secondary"]
        ).pack(anchor="w", padx=12, pady=(8, 4))

        tap_frame = ctk.CTkFrame(key_card, fg_color="transparent")
        tap_frame.pack(fill="x", padx=12, pady=(0, 4))

        ctk.CTkLabel(tap_frame, text="Your Tap Key",
                     font=ctk.CTkFont(size=11),
                     text_color=t["secondary"]).pack(anchor="w")

        self.tap_key_btn = ctk.CTkButton(
            tap_frame, text=KeyboardHookManager.get_key_display_name(self.settings.send_key),
        ...
        self.tap_key_status = ctk.CTkLabel(
            tap_frame, text="● idle",
            font=ctk.CTkFont(size=10),
            text_color=t["faded"],
        )
        ...
        ctk.CTkLabel(
            tap_frame, text="Tip: Turn on Tap Mode below, then hold this key. (Short press = Dot, Long press = Dash)",
            font=ctk.CTkFont(size=10), text_color=t["faded"]
        ).pack(anchor="w", pady=(4, 10))

        # ── Row 4: Message Composer ──
        msg_card = self._card(self.scroll, "Message")
        msg_card.pack(fill="x", padx=20, pady=4)

        self.msg_entry = ctk.CTkEntry(
            msg_card, height=36, corner_radius=8,
            fg_color=t["bg"], border_color=t["border"],
            text_color=t["text"], border_width=1,
            font=ctk.CTkFont(size=14),
            placeholder_text="Type a message to encode & send…",
        )
        self.msg_entry.pack(fill="x", padx=12, pady=(8, 4))
        self.msg_entry.bind("<KeyRelease>", self._on_msg_type)

        self.morse_preview = ctk.CTkLabel(
            msg_card, text="",
            font=ctk.CTkFont(family="Consolas", size=14),
            text_color=t["dot"],
            anchor="w", justify="left", wraplength=640,
        )
        self.morse_preview.pack(fill="x", padx=12, pady=(0, 4))

        self.tap_display = ctk.CTkLabel(
            msg_card, text="",
            font=ctk.CTkFont(family="Consolas", size=14),
            text_color=t["dot"],
            anchor="w", justify="left", wraplength=640,
        )
        self.tap_display.pack(fill="x", padx=12, pady=(0, 4))
        ...
        self.send_btn = ctk.CTkButton(
            btn_frame, text="Send ▸", width=100, height=32,
        ...
        self.tap_btn = ctk.CTkButton(
            btn_frame, text="⌨ Tap Mode: OFF", width=140, height=32,
            corner_radius=8, font=ctk.CTkFont(size=12),
            fg_color=t["card_hover"], text_color=t["text"],
            border_width=1, border_color=t["border"],
            command=self._toggle_tap_mode,
        )
        ...
        popup_card.pack(fill="x", padx=20, pady=4)

        corner_frame = ctk.CTkFrame(popup_card, fg_color="transparent")
        corner_frame.pack(fill="x", padx=12, pady=(8, 4))

        ctk.CTkLabel(corner_frame, text="Popup corner:",
                     font=ctk.CTkFont(size=12),
                     text_color=t["secondary"]).pack(side="left")

        self.corner_menu = ctk.CTkOptionMenu(
            corner_frame, width=140, height=28,
            values=["top-left", "top-right", "bottom-left", "bottom-right"],
            fg_color=t["bg"], button_color=t["card_hover"],
            text_color=t["text"], dropdown_fg_color=t["card"],
            font=ctk.CTkFont(size=12),
            command=lambda v: setattr(self.settings, "popup_corner", v),
        )
        ...
        for label, var, attr in [
            ...
            ctk.CTkCheckBox(
                chk_frame, text=label,
                font=ctk.CTkFont(size=12),
                text_color=t["secondary"],
                fg_color=t["accent"],
                variable=var,
                command=lambda a=attr, v=var: self._on_check_change(a, v),
            ).pack(anchor="w", pady=2)

        # Duration
        self._slider_row(popup_card, "Auto-hide after", "duration_slider",
                         0, 30, self.settings.popup_duration,
                         "s", self._on_duration_change, pad_bottom=10)

        # ── Row 6: History ──
        hist_card = self._card(self.scroll, "Message History")
        hist_card.pack(fill="x", padx=20, pady=4)

        self.history_text = ctk.CTkTextbox(
            hist_card, height=120, corner_radius=8,
            fg_color=t["bg"], text_color=t["text"],
            border_width=1, border_color=t["border"],
            font=ctk.CTkFont(family="Consolas", size=11),
        )
        self.history_text.pack(fill="x", padx=12, pady=8)
    ...
    def _card(self, parent, title: str) -> ctk.CTkFrame:
        """Create a card with a title label."""
        t = self.theme
        card = ctk.CTkFrame(
    ...
    def _slider_row(self, parent, label, attr_name,
                    min_val, max_val, current, suffix, callback,
                    pad_bottom=8):
        """Create a labeled slider row inside a card."""
        t = self.theme
        frame = ctk.CTkFrame(parent, fg_color="transparent")
        frame.pack(fill="x", padx=12, pady=(2, pad_bottom))
        ...
        slider = ctk.CTkSlider(
            ...
            command=lambda v: self._slider_callback(callback, v, value_label, suffix),
        )
        slider.set(current)
    ...
    def _on_freq_change(self, v):
        self.settings.frequency = int(v)
        self.audio.frequency = int(v)
        self.settings.save()
    ...
    def _on_wpm_change(self, v):
        self.settings.wpm = int(v)
        self.audio.wpm = int(v)
    ...
    def _on_dot_change(self, v):
        self.settings.dot_threshold = int(v)
        self.tap.update_timing(dot_threshold=int(v))
        self.settings.save()

    def _on_letter_gap_change(self, v):
        self.settings.letter_gap = int(v)
        self.tap.update_timing(letter_gap=int(v))
        self.settings.save()

    def _on_word_gap_change(self, v):
        self.settings.word_gap = int(v)
        self.tap.update_timing(word_gap=int(v))
        self.settings.save()
    ...
    def _refresh_timing_sliders(self):
        ...
        self.tap.update_timing(
            self.settings.dot_threshold,
            self.settings.letter_gap,
            self.settings.word_gap,
        )

    def _on_duration_change(self, v):
        self.settings.popup_duration = int(v)
        self.settings.save()

    def _on_check_change(self, attr, var):
    ...
    def _capture_key(self):
        btn = self.tap_key_btn
        btn.configure(text="Press any key…", fg_color=self.theme["accent"],
                      text_color=self.theme["bg"])

        def on_captured(key_name: str):
            display = KeyboardHookManager.get_key_display_name(key_name)
            btn.configure(text=display, fg_color=self.theme["bg"],
                          text_color=self.theme["text"])
            self.settings.send_key = key_name
            self.kb.set_tap_key(key_name)
            if self.mini_window:
                self.mini_window.tap_indicator.configure(text=f"Hold {display} to tap")
            self.settings.save()

        self.kb.capture_next_key(on_captured)
    ...
    def _on_msg_type(self, event=None):
        ...
        if text.strip():
            ...
            self.morse_preview.configure(text=morse)
        else:
            self.morse_preview.configure(text="")

    def _send_message(self):
        # If tap mode is active, use the tapped morse
        ...
        valid, msg = validate_callsign(cs)
        if not valid:
            messagebox.showwarning("Invalid callsign", msg)
            return
        ...
        def do_send():
            ...
            if success:
                msg_obj = MorseMessage(
                    callsign=cs, morse=morse, text=text,
                    timestamp=time.time(), direction="out"
                )
    ...
    def _toggle_tap_mode(self):
        ...
        if self._tap_mode_active:
            ...
            self.tap.stop()
            self.audio.stop_tone()  # Ensure tone stops if we exit tap mode abruptly
            self.tap_btn.configure(
                text="⌨ Tap Mode: OFF",
                fg_color=self.theme["card_hover"],
                text_color=self.theme["text"],
            )
            # Reset visual state of key button
    ...
    def _update_tap_display(self, morse: str):
        ...
        if self.mini_window:
            self.mini_window._update_display(morse)
    ...
    def _update_key_status(self, pressed: bool):
        ...
        # Visually light up the button while transmitting if in tap mode
        if self._tap_mode_active:
            if pressed:
        ...
        if self.mini_window:
            self.mini_window._update_indicator(pressed)

    # ── Key event handlers (Live Side-Tone) ────────────────────

    def _on_tap_key_down(self):
        if self._tap_mode_active:
            self.tap.on_key_down()
            self.audio.start_tone()  # Play local beep instantly!

    def _on_tap_key_up(self):
        if self._tap_mode_active:
            self.tap.on_key_up()
            self.audio.stop_tone()   # Stop local beep instantly!

    # ── Network event handlers ─────────────────────────────────

    def _on_message_received(self, msg: dict):
        morse = msg.get("morse", "")
        text = msg.get("text", MorseTranslator.decode(morse))
        callsign = msg.get("callsign", "???")
        timestamp = msg.get("timestamp", time.time())

        # Check callsign conflict
        my_cs = self.callsign_entry.get().strip().upper()
        conflict = (callsign.upper() == my_cs)

        msg_obj = MorseMessage(
            callsign=callsign, morse=morse, text=text,
        ...
        if self.settings.play_audio:
            def on_char(i, c):
                self.after(0, lambda: self._highlight_popup_char(i))
            self.audio.play_morse_async(morse, self.settings.wpm,
                                        on_char_played=on_char)
    ...
    def _on_handshake(self, callsign: str, ip: str):
        ...
        if callsign.upper() == my_cs:
            self.after(0, lambda: messagebox.showwarning(
                "Callsign Conflict",
                f"Peer '{callsign}' has the same callsign as you!\n"
                f"Please choose a different callsign."
            ))
        self.after(0, lambda: self._on_net_status(
            f"Connected: {callsign} @ {ip}"
        ))

    def _on_net_status(self, status: str):
        self.status_label.configure(text=status)
        if "Listening" in status or "Connected" in status or "Sent" in status:
    ...
    def _on_peer_discovered(self, callsign: str, ip: str, port: int):
        self.peers_label.configure(
            text=f"Found: {callsign} @ {ip}:{port}  (click to set)",
            text_color=self.theme["text"], cursor="hand2",
        )
    ...
    def _show_popup(self, callsign: str, morse: str, text: str,
                    timestamp: float, conflict: bool = False):
        ...
        popup = MorsePopup(
            ...
            theme=self.theme,
            on_repeat=lambda: self._repeat_popup_audio(morse),
            on_reply=lambda: self._reply_to_sender(callsign),
            on_close=lambda: setattr(self, "_current_popup", None),
        )

        display_text = text
        if conflict:
            display_text = f"[CONFLICT] {text}"
    ...
    def _highlight_popup_char(self, index: int):
        if self._current_popup:
            self._current_popup.highlight_char(index)
    ...
    def _reply_to_sender(self, callsign: str):
        """Focus the message entry to compose a reply."""
        self.deiconify()
        self.focus_force()
    ...
    def _add_to_history(self, msg: MorseMessage):
        ...
        self.settings.history = self.settings.history[:50]  # keep last 50
        self.settings.save()
        self._refresh_history()
    ...
    def _test_tone(self):
        self.audio.play_test_tone(300)

    def _toggle_listen(self):
        if self.network._running:
            self.network.stop_listening()
            self.listen_btn.configure(text="Start Listen", fg_color=self.theme["card_hover"])
        else:
    ...
    def _minimize_to_tray(self):
        self.withdraw()
        self.app.show_tray_notification("MorseRelay", "Running in background")
...
class MiniWindow(ctk.CTkToplevel):
    """Graceful, minimalist rounded rectangle for quick morse sending."""
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            header, text=self.settings.callsign,
        ...
        expand_btn = ctk.CTkLabel(
            ...
            cursor="hand2", text_color=t["secondary"],
        )
        expand_btn.pack(side="right")
        expand_btn.bind("<Button-1>", lambda e: self._expand())

        # Divider
        ctk.CTkFrame(frame, fg_color=t["border"], height=1).pack(
        ...
        self.morse_label = ctk.CTkLabel(
            ...
            text_color=t["dot"], anchor="center", wraplength=280,
        )
        self.morse_label.pack(fill="x", padx=12, pady=(8, 2))

        self.text_label = ctk.CTkLabel(
            frame, text="",
            font=ctk.CTkFont(family="Consolas", size=11),
            text_color=t["faded"], anchor="center",
        )
        self.text_label.pack(fill="x", padx=12, pady=(0, 4))

        # Tap indicator
        self.tap_indicator = ctk.CTkLabel(
            frame, text=f"Hold {KeyboardHookManager.get_key_display_name(self.settings.send_key)} to tap",
            font=ctk.CTkFont(size=10),
            text_color=t["secondary"],
        )
        ...
        ctk.CTkButton(
            btn_frame, text="Send ▸", height=28, corner_radius=6,
        ...
        ctk.CTkButton(
            btn_frame, text="Clear", height=28, width=60, corner_radius=6,
            font=ctk.CTkFont(size=11),
            fg_color=t["card_hover"], text_color=t["text"],
            border_width=1, border_color=t["border"],
            command=self._clear,
    ...
    def _update_indicator(self, pressed: bool):
        if pressed:
            self.tap_indicator.configure(
                text="● transmitting",
                text_color=self.theme["accent"],
            )
        else:
            self.tap_indicator.configure(
                text=f"Hold {KeyboardHookManager.get_key_display_name(self.settings.send_key)} to tap",
    ...
    def _send(self):
        ...
        if not morse:
            return
        text = self.tap.get_text()
        target = self.settings.target_ip
        if not target:
            return

        def do_send():
            self.dashboard.network.send_message(target, morse, text)
